...
const MatchesEntities = () => {
  ...
  // TODO: API is not support order hit on shipment focus
  ...
  const matchedContainerIds = keys.filter(
    key =>
      key.includes(`-${CONTAINER}`) && mapping.entities?.containers?.[matches.entity?.[key]?.id]
  );
  const containerIds = [
    ...new Set(
      matchedBatchIds.map(id => {
  ...
  // TODO: API is not support container on order focus
  const matchedContainer = matchedContainerIds.length;
  const isUnselectContainers =
    intersection(selectedContainerIds, containerIds).length === matchedContainer;
