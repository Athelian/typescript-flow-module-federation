...
import { productProviderAutoDateQuery } from './query';
    ...
    emitter.addListener(
      ...
      async ({
        ...
        if (inParentEntityForm) {
          ...
          logger.warn('query productProvider data for id', client);
          // TODO: This flag will be used for showing loading on UI
          emitter.emit('LIVE_VALUE_PROCESS', true);
          const { data } = await client.query({
            query: productProviderAutoDateQuery,
            variables: { id: entityId },
            fetchPolicy: 'cache-first',
          });
          emitter.emit('LIVE_VALUE_PROCESS', false);

          let date = mappingDate({ field, task, values: data.productProvider });
          if (autoDateDuration) {
            date = calculateDate({
              date,
              duration: autoDateDuration.metric,
